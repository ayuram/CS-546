...
use_prefilter = True
if use_prefilter:
    train_k = load_from_disk("conala_id")
    print(train_k)
else:
    train = load_dataset("dtruong46me/code-generation-py", "conala", trust_remote_code=True)["train"]
    
    def prepare_self(batch):
        answers = []
        choices_batch = []
        for i in range(len(batch["prompt"])):
            choices = choices = [batch["completion"][i]] + random.sample(batch["completion"], 3)
            random.shuffle(choices)
            choice_str = f"A: {choices[0]}, B: {choices[1]}, C: {choices[2]}, D: {choices[3]}"
            choices_batch.append(choices)
            answers.append(f"Question: {batch['prompt'][i]}. Choices: {choice_str}. Give ONLY the letter indicating your response. Answer: ")
    ...
    print(train_k)
    train_k.save_to_disk("conala_id")
...
def process_rtune(batch):
    ...
    for i in range(len(batch["prompt"])):
        ...
        if len(distractors) > 0:
            ...
            answers.append(f"Question: {batch['prompt'][i]}. Answer: {wrong}. Are you sure you answered the question correctly based on your internal knowledge? I am unsure.")
        answers.append(f"Question: {batch['prompt'][i]}. Answer: {answer}. Are you sure you answered the question correctly based on your internal knowledge? I am sure.")
    return {"text": answers}
...
training_args = TrainingArguments(
    ...
    logging_steps=50,
    output_dir='./results-conala',
    evaluation_strategy='no',
    logging_dir='./logs',
    report_to='wandb',
    run_name='llama-3.2-1b-lora-conala-finetune',
    fp16=True,
)
